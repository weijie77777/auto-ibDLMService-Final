...
def test_model(pre_Y, test_label, threhold, global_y_max, timestep):
    """
    生成图表并上传OSS
...
# ========== 改造：主预测函数，支持OSS路径 ==========
def finalpredict_from_oss(file_x_url, file_add_url, file_global_url, feature_selector_url, model_url, threhold, timestep, only_local, size=7):
    """
    主预测函数（Flask调用入口）
    :param file_x_url: OSS上的微观特征文件URL
    :param file_add_url: OSS上的中观特征文件URL
    ...
    feature_selector = FeatureSelector(input_dim=all_X.size(2) - 1, output_dim=10)
    fs_path = oss_client.download_model(feature_selector_url, Config.TEMP_DIR)
    ...
    gru_path = oss_client.download_model(model_url, Config.TEMP_DIR)
    ...
    if timestep == None:
        ...
        with torch.no_grad():
            X_sample = all_X[timestep-8, :, :-1]
            X_last = all_X[timestep-8, :, -1].unsqueeze(1)
            X_selected_i = feature_selector(X_sample)
            X_selected = torch.cat((X_selected_i, X_last), dim=1)
            selected_features_test.append(X_selected)
    ...
    # 5. 生成图表并上传OSS
    event_num, event_predict_num, recall, precision, accuracy, image_url = test_model(y_pred, test_Y, threhold, y_max, timestep)
