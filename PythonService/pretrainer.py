...
def split_windows(size, file_x_url, file_add_url, file_global_url, only_local):
    ...
    try:
        ...
        df_y = pd.read_csv(local_global, usecols=['node_count'])
        ...
        for i in range(len(all_data) - size - 1):
            ...
            if df_add is not None:
                ...
                Y1.append(all_data[i:i + size, [len(head_x)]])
...
# ========== 改造：主预测函数，支持OSS路径 ==========
def pretrain_featureSelector(file_x_url, file_add_url, file_global_url, num_epochs, only_local, size=7):
