<template>
  <div class="train-model-predict-page">
    <NavBar />
    <div class="container">
      ...
      <div class="buttons">
        <input value="Predict" type="submit" class="train-model-predict-submit-btn" @click="handlePredict">
      </div>
      <span class="train-model-predict-span1"></span>
      <div class="spinner train-model-predict-spinner" v-show="loading"></div>
      <span class="train-model-predict-span2"></span>
...
import { TrainModelPredict } from '@/api/predict';
...
const handlePredict = async () => {
  ...
  try {
    const res = await TrainModelPredict();
    if (res === 1) {
      span1.value = '';
