<template>
  <div class="embedding-step">
    <StepDescription 
      text="Train the auto-ibDLM model. The model learns from historical evolution data to predict the development of public events."
    />
    ...
    <div class="embedding-content">
      <div v-if="stepStatus === 'pending'" class="action-card">
        ...
        <h4 class="action-title">Fine-tuning Models</h4>
        <!-- 新增输入项 -->
        ...
        <button class="action-button" @click="$emit('train', 4, epochs)" >
          Start Training
        </button>
      </div>
      ...
      <div v-else-if="stepStatus === 'processing'" class="processing-state">
        ...
        <h4 class="processing-title">Training Model...</h4>
      ...
      <div v-else class="result-state">
        <div class="result-container">
          <div class="result-content">
            <h3 class="result-title">Model Training<br/>Completed</h3>
            <p class="result-description">
              The auto-ibDLM model has been successfully trained. The model is ready to predict the development of public events.
            </p>
            <button class="result-button" @click="downloadTrain">
              ...
              Export auto-learning layer
            </button>
            <button class="result-button" @click="downloadTrainModel">
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
                <polyline points="7 10 12 15 17 10"></polyline>
                <line x1="12" y1="15" x2="12" y2="3"></line>
...
try {
  ...
  link.download = 'Train_FeatureSelector.pth'  // 下载文件名
...
const downloadTrainModel = async() => {
try {
  const response = await request({
    url: '/network//downloadTrainModel',
    method: 'GET',
    responseType: 'blob'  // 关键：指定响应类型为二进制流
  })
  ...
  const link = document.createElement('a')
  link.href = URL.createObjectURL(blob)
  link.download = 'Train_gru.pth'  // 下载文件名
  document.body.appendChild(link)
  link.click()
  document.body.removeChild(link)
  URL.revokeObjectURL(link.href)
