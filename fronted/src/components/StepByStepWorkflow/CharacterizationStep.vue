<template>
  <div class="characterization-step">
    <StepDescription 
      text="Starting from the snapshot-based network structure, we extract network features. You may set a division range for network snapshots and opt to extract only node-level features or both node-level and network-level features."
    />
    ...
    <div class="characterization-content">
      ...
      <div class="input-group" style="margin: 10px 0; display: flex; align-items: center; gap: 8px;">
      <label style="font-size: 14px; color: #333; font-weight: 500;">Characterization Type:</label>
      <select 
        v-model="analysisType" 
        style="padding: 8px; border: 1px solid #ddd; border-radius: 4px; background: #fff; width: 200px;"
      >
        <option value="only-node">only-node</option>
        <option value="node+network">node+network</option>
      </select>
    </div>
...
const analysisType = ref('node+network'); // 默认node+network

const emit = defineEmits(['analyze']);

const handleAnalyze = () => {
  const timeParam = `${timeValue.value}${timeUnit.value}`
  const typeParam = analysisType.value === 'only-node' ? 0 : 1
  emit('analyze', 2, timeParam, typeParam)
}
...
// };
const initChart = async () => {
  // 第一步：等待 v-else 容器和 chartRef 都渲染完成
  if (!containerRef.value || !chartRef.value) {
    console.warn('容器未就绪，等待后重试');
    await nextTick();
    setTimeout(initChart, 100); // 延迟重试，确保DOM完全渲染
    return;
  }
...
watch([() => props.featureData, () => containerRef.value, () => chartRef.value], async () => {
  if (hasData.value && containerRef.value && chartRef.value) {
