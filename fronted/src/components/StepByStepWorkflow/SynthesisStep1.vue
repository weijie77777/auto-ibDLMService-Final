    ...
    <div class="synthesis-content">
      ...
      <div v-else class="result-state">
        ...
        <div class="result-realcontainer" v-if="isShow==true">
          ...
          <NetworkGraph1 :data="graphData" class="networkGraph"/>
        </div>
      </div>
    </div>
  </div>
...
import NetworkGraph1 from './NetworkGraph1.vue';
...
//  computed 会自动跟踪 props.data 的变化，实时更新
...
try {
  const response = await request({
    url: '/network/downloadExampleEdgeList',
    method: 'GET',
...
// 组件挂载时打印 props
onMounted(() => {
  console.log('子组件 SynthesisStepA 接收到的 props：', props);
  console.log('子组件接收到的 data：', props.data);
});
</script>
