<template>
  <div class="alignment-step">
    <StepDescription 
      text="Predict public events using the trained auto-ibDLM model. You can set a threshold to determine the node increment for event triggering, and you can also enter the specific time step you want to predict. If you choose not to enter it, the platform will predict the entire time range by default."
    />
    ...
    <div class="alignment-content">
      <div v-if="stepStatus === 'pending'" class="action-card">
        ...
        <div class="input-group">
          <label for="Predict TimeStep">predict timestep</label>
          <input
            id="timestep"
            ...
            :min="min_timestep"
            :max="max_timestep"
            :placeholder="`Please input timestep in (${min_timestep} ~ ${max_timestep})`"
          >
        </div>
        <div class="input-group">
        ...
        <button class="action-button" @click="$emit('finalize', 4, timestep, threshold)">
          Start Prediction
        </button>
      </div>
...
const min_timestep = ref(8);
const max_timestep = ref(7);
const STORAGE_KEY = 'step-by-step-workflow-state';
onMounted(() => {
  const state = JSON.parse(localStorage.getItem(STORAGE_KEY));
  if (state) {
    max_timestep.value = state.featureData.snapshots.length-1;
  }
});
</script>
